...
export interface OvalTableProps {
  ...
  currentVariant: import('@/lib/types').GameVariant;
  onOpenChat: () => void;
  onSitBack: () => void;
  onSitOut: () => void;
  onTakeSeat: () => void;
...
export function OvalTable({
  ...
  drawUI, actionPanel, unreadCount, playerStats, currentVariant,
  ...
  // Equity calculation — runs automatically at showdown
  const isShowdown = gameState?.phase === 'showdown';
  const showdownCards = isShowdown ? (activeResult?.showdownCards ?? null) : null;
  const equityResults = useEquity(showdownCards, gameState?.communityCards ?? [], currentVariant);
          ...
          <div style={{ position: 'relative', width: '76%', paddingBottom: '40%' }}>
            ...
            {seatedOpponents.map(({ player, seatIndex }) => (
              <OvalSeat
                ...
                equity={isShowdown ? equityMap[player.sessionToken] : undefined}
              />
            ))}
            ...
            <div style={{ position: 'absolute', top: youPos.top, left: youPos.left, transform: 'translate(-50%, -50%)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, zIndex: 10 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
                ...
                {isShowdown && equityMap[mySessionToken] !== undefined && (

...
export function TexasCalculator() {
  ...
  const setCard = (slot: string, card: Card) => {
    if (slot.startsWith('h')) {
      ...
      setHeroCards(prev => { const n = [...prev] as [Card, Card]; n[idx] = card; return n; });
    } else {
      const idx = parseInt(slot[1]);
  ...
  const calcEquity = useCallback(() => {
    ...
    setRunning(true);
    setTimeout(() => {
      const h = heroCards as Card[];
      const board = boardCards.filter(Boolean) as Card[];
      const N = 5000;
      let wins = 0, ties = 0;
      for (let i = 0; i < N; i++) {
        ...
        const hs = bestHand(h, sb);
        let ob = 0;
        for (let o = 0; o < numOpp; o++) {
          const oh = [deck[di++], deck[di++]];
          const s = bestHand(oh, sb);
          if (s > ob) ob = s;
        }
        if (hs > ob) wins++;
        else if (hs === ob) ties++;
  ...
  const slotLabel: Record<string, string> = { h0: '1. karta', h1: '2. karta', b0: 'Flop 1', b1: 'Flop 2', b2: 'Flop 3', b3: 'Turn', b4: 'River' };

  return (
    <div style={{ background: '#1a1a2a', border: '1px solid rgba(212,175,55,0.2)', borderRadius: 16, padding: '1.5rem', maxWidth: 700 }}>

      {/* Hero cards + opponents */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '1.5rem' }}>
        <div>
          <div style={{ fontSize: '0.75rem', fontWeight: 600, color: 'rgba(212,175,55,0.6)', letterSpacing: '0.07em', textTransform: 'uppercase', marginBottom: 10 }}>Twoje karty (hero)</div>
          <div style={{ display: 'flex', gap: 8 }}>
            {([0, 1] as const).map(i => (
              <div key={i} onClick={() => heroCards[i] ? clearCard(`h${i}`) : openPicker(`h${i}`)} style={{ cursor: 'pointer' }}>
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        <div style={{ fontSize: '0.75rem', fontWeight: 600, color: 'rgba(212,175,55,0.6)', letterSpacing: '0.07em', textTransform: 'uppercase', marginBottom: 10 }}>Karty wspólne (opcjonalne)</div>
      ...
      {result && (
        <div style={{ marginBottom: '1rem' }}>
          ...
          <div style={{ marginTop: 12, padding: '10px 14px', background: '#0f1f3d', borderLeft: '3px solid #3b82f6', borderRadius: 6, fontSize: 13, color: '#93c5fd' }}>
            ...
            {boardCards.filter(Boolean).length === 0 ? 'Brak kart wspólnych — equity pre-flop.' : boardCards.filter(Boolean).length < 5 ? `${boardCards.filter(Boolean).length} kart wspólnych zdefiniowanych.` : 'Pełna plansza — wynik deterministyczny.'}
          </div>
        </div>
      )}
